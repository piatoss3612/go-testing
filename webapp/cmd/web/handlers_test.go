...
func Test_application_Handlers(t *testing.T) {
	...
	for _, e := range testRoutes {
		...
		if resp2.StatusCode != e.expectedFirstStatusCode {
			t.Errorf("for %s: expected first status to be %d but got %d", e.name, e.expectedFirstStatusCode, resp.StatusCode)
		}
	}
}
...
func Test_app_UploadProfilePic(t *testing.T) {
	...
	// create a byte.Buffer to act as the request body
